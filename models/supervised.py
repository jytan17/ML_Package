...
from cvxpy import quad_form, multiply, Variable, Problem, Minimize, pos, norm
import random
...
class FeedForward():
    ...
    def fit(self, Xtr, ytr, epochs, mini_batch_size, eta, Xva = None, yva = None):
        training_data = [(x.reshape(-1, 1), y.reshape(-1, 1)) for x, y in zip(Xtr, ytr)]
    ...
    def predict(self, X):
        data = [x.reshape(-1, 1) for x in X]
        preds = np.array([np.argmax(self.feedforward(x)) for x in data])
        return preds
